...
const CNEMonitor = (() => {
  ...
  function setThemeIcon(isDarkMode) {
    const sunIcon = document.querySelector('#toggleTheme .icon-sun');
    const moonIcon = document.querySelector('#toggleTheme .icon-moon');
    if (!sunIcon || !moonIcon) {
      return;
    }
  ...
  function loadAnalytics() {
    ...
    script.onerror = () => {
      console.warn('Unable to load Google Analytics script.');
    };
...
async function loadTranslations() {
  ...
  const response = await fetch(TRANSLATIONS_URL, { cache: 'force-cache' });
  if (!response.ok) {
...
function getTranslationTemplate(lang, key) {
  if (translationsCache && translationsCache[lang] && translationsCache[lang][key]) {
    return translationsCache[lang][key];
  }
  if (lang === DEFAULT_LANG && Object.prototype.hasOwnProperty.call(DEFAULT_TRANSLATIONS, key)) {
...
function formatHeroSuptitle(lang, days) {
  ...
  const template = getTranslationTemplate(safeLang, 'hero_suptitle');
...
function formatCredDocs(lang) {
  const safeLang = getSafeLang(lang);
  const template = getTranslationTemplate(safeLang, 'cred_docs');
  return template.replace(CREDIBILITY_DOCS_PLACEHOLDER, String(CREDIBILITY_DOCS_COUNT));
}

function formatCredUpdate(lang) {
  const safeLang = getSafeLang(lang);
  const template = getTranslationTemplate(safeLang, 'cred_update');
...
async function updateLanguage(lang) {
  ...
  try {
    const loadedTranslations = translationsCache || (await loadTranslations());
    langTranslations =
      loadedTranslations[resolvedLang] ||
      loadedTranslations[DEFAULT_LANG] ||
      loadedTranslations[SUPPORTED_LANGS[0]] ||
      {};
  } catch (error) {
    console.warn('Unable to load translations:', error);
    langTranslations = translationsCache?.[DEFAULT_LANG] || {};
  }
...
document.addEventListener('DOMContentLoaded', () => {
  if (currentLang !== DEFAULT_LANG) {
    scheduleIdle(() => {
      void updateLanguage(currentLang);
    });
  } else {
    scheduleIdle(() => {
      observeHighlights();
